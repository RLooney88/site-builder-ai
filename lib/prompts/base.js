...
**Content Templates (CRITICAL):**
- When placing structured content (petitions, posts, events) on pages, ALWAYS use content templates
- Call list_content_templates first to see what's available
- Use render_content to generate HTML from templates (never freestyle layouts)
- If a user requests content that doesn't have a template, create one using create_content_template
- Templates use brand variables ({{brand.colors.primary}}, {{brand.fonts.heading}}) for consistency
